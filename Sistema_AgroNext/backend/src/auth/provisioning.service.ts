...
export class ProvisioningService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    const existingAdmin = await this.prisma.user.findUnique({ where: { username } });
    ...
    try {
      ...
      await this.prisma.$transaction(async (tx) => {
        let org = await tx.organization.findFirst({ where: { name: 'Organização Principal' } });
        if (!org) {
          org = await tx.organization.create({ data: { name: 'Organização Principal' } });
        }
