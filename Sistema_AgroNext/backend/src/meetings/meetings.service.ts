  ...
  async authorizeActionToTask(
    ...
    return this.prisma.$transaction(async (tx) => {
      const action = await tx.meetingAction.findFirst({
        where: {
          id: actionId,
          minute: { meeting: { project: { organizationId: orgId } } },
        },
      });
      ...
      if (action.taskId || action.status === "AUTHORIZED") {
        throw new BadRequestException(
          "Esta ação já foi transformada em tarefa.",
        );
      }

      const user = await tx.user.findFirst({
        where: { id: resolvedUserId, organizationId: orgId },
      });
